...
printf("============ysysysysysys==========%d==%d==\n",fmt.fmt.pix.width,fmt.fmt.pix.height);
    ...
    {   
        printf("****************8899999********\n");
        if(errno == EINVAL){
            printf("not supposrt format 800*480\n");
        }
    }
printf("============ysysysysysys==============\n");
...
int linux_v4l2_get_fream(FrameBuffer *freambuf_t)
{
	struct timeval tv;
	struct v4l2_buffer buf;
	fd_set fds; 
